...
profiler node = NN $ JSExpression [ fromRight $ flip parse "" $
  concat [ "window." ++ identifier "result" ++ " = window." ++ identifier "result" ++ " || {}; "
         ...
         , identifier "format" ++ " = function(x) { return 'time: ' + (x.time / 100) + 'sec   ' + x.fname + ' ' + x.name + ' (line: ' + x.line + ', col: ' + x.col + '): count: ' + x.count + '     ' + x.linestr; }; "
         , "if (window.hasOwnProperty('" ++ identifier "result_interval" ++ "')) { "
         , "  clearInterval(window." ++ identifier "result_interval" ++ ");"
         , "}"
